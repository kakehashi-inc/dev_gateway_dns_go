...
セキュリティソフトやファイアウォールが受信接続をブロックする場合があります。`--listen` で指定したIPアドレスのヘルスチェックが失敗する場合は、セキュリティソフトなどのファイアウォール設定で以下のパスの受信接続を許可してください。

- アプリ直指定の場合: `<プロジェクトルート>/__debug_bin<数字>`
- ワイルドカードの場合: `<プロジェクトルート>/__debug_bin*`
